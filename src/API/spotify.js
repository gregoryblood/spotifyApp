...
const refreshToken = async() => {
  const { data } = await axios.get(`https://musicmajik.herokuapp.com/refresh_token`);
...
export const getAccessToken = () => {
  ...
  if (Date.now() - getExpTime() > 1000 * 60 * 60) {
    logout();
    window.location.href = "https://musicmajik.herokuapp.com";
...
export var token = getAccessToken();
